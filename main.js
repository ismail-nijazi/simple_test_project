...
function setupGame() {
  ...
  cards = shuffle([...CARD_PAIRS, ...CARD_PAIRS]);
  flippedCards = [];
  matchedIds = [];
  moves = 0;
  updateMoveCounter();
...
function renderBoard() {
  ...
  cards.forEach((card, idx) => {
    ...
    if (matchedIds.includes(card.id)) cardElem.classList.add('flipped');
    cardElem.innerHTML = `
...
function unflipCards(indices) {
  const cardElems = document.querySelectorAll('.card');
  indices.forEach(i => cardElems[i].classList.remove('flipped'));
}
...
function checkForMatch() {
  ...
  if (cards[i1].id === cards[i2].id) {
    ...
    flippedCards = [];
    if (matchedIds.length === CARD_PAIRS.length) {
